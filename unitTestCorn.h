...
class unitTestCorn {
 ...
 protected:
  ...
  void testConstructor() {
    ...
    // correct setting.
    if (corn.getSellPrice() != 15 && corn.getBuyPrice() != 13 &&
        corn.getSellTime() == 5 && corn.getHighYieldPrice() == 19) {
  ...
  void testApplyHighYield() {
    ...
    // If statement to check if cornHighYieldApplied is true and money has
    // decreased.
  ...
  void testEvent() {
    ...
    // Note that when random num gen taken out, below works. Otherwise, pig
